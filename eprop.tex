...
When comparing the performance of models trained with eProp on the SHD dataset with results obtained using BPTT~\citep{Zenke2021a} we found no considerable differences for the \num{256} neuron models (figure~\ref{fig:shd_performance}). 
...
Figure~\ref{fig:training_time} shows how the time taken to train recurrent LSNNs using eProp with GeNN compares against training recurrent LIF networks using BPTT with PyTorch code from \citet{Zenke2021a}.
