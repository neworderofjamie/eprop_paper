...
For short input sequences, training using GeNN is slower but, the runtime of GeNN simulations increases slower with sequence length and, because BPTT requires the state of the model to be recorded every time step, the memory requirements are such that PyTorch is unable to train models larger than \num{256} neurons on \num{1370} timestep input sequences with our chosen batch size of \num{512}.
